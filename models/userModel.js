...
userSchema.pre('save', async function (next) {
  ...
  // we used await coz it's a promse
  // hash is the asynchronous version

  this.passwordConfirm = undefined; // remove passwordConfirm field to save in db
  // still required to be inputed but not persisted in the database

  next();
});
// modifiying changed at
userSchema.pre('save', function (next) {
  if (!this.isModified('password') || this.isNew) return next();
  this.passwordChangedAt = Date.now() - 1000; // we subtracted 1 sec coz of delay of savin a document;
  next();
});
...
userSchema.methods.createPasswordResetToken = function () {
  ...
  // saving the hashed token into the databas
